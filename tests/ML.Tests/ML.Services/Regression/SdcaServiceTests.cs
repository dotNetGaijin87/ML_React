...
{
    [TestFixture]
    public class SdcaServiceTests
    {
        ISdcaModelBuilderFactory _fakeModelBuilderFactory;
        IRegressionPredictionEngine _fakePredictionEngine;
        IMlRepository _fakeMlDataLoader;
        IDataPathRegister _fakeDataPathRegister;
        IModelBuilder<SdcaRegressionModelBuilderOptions> _fakeModelBuilder;


        private void ReinitializeFakes()
        {
            _fakeModelBuilderFactory = Substitute.For<ISdcaModelBuilderFactory>();
            ...
            _fakeModelBuilder = Substitute.For<IModelBuilder<SdcaRegressionModelBuilderOptions>>();
            _fakeModelBuilder.Build(default, default).ReturnsForAnyArgs((null, new RegressionModelBuilderOutput()));
            _fakeModelBuilderFactory.CreateSdcaModelBuilder(default).ReturnsForAnyArgs(_fakeModelBuilder);
        }
        ...
        //    // ARRANGE
        //    ReinitializeFakes();
        //    var service = new SdcaRegressionService(_fakeModelBuilderFactory,_fakePredictionEngine, _fakeMlDataLoader, _fakeDataPathRegister,new MLContext());

        //    // ACT
        //    service.Create(new CreateSdcaRegressionModelOptions()
        //    {
        ...
        public void Create_BuildCalled()
        {
            // ARRANGE
            ReinitializeFakes();
            var service = new SdcaRegressionService(_fakeModelBuilderFactory, _fakePredictionEngine, _fakeMlDataLoader, new MLContext());
        ...
        public void Create_CorrectDataSupplied_ReturnsCorrectData()
        {
            ...
            var fakeModelBuilderFactory = Substitute.For<ISdcaModelBuilderFactory>();
            var fakeModelBuilder = Substitute.For<IModelBuilder<SdcaRegressionModelBuilderOptions>>();
            fakeModelBuilder.Build(default, default).ReturnsForAnyArgs((default, expectedResult));
            fakeModelBuilderFactory.CreateSdcaModelBuilder(default).ReturnsForAnyArgs(fakeModelBuilder);
            var service = new SdcaRegressionService(fakeModelBuilderFactory, _fakePredictionEngine, _fakeMlDataLoader, new MLContext());

            // ACT
            var result = service.Create(new CreateSdcaRegressionModelOptions());
